    ...
    for (x = 0; x != width; ++x) {
      ...
      for (int filterY = 0; filterY < MASK_Y; filterY++){
        for (int filterX = 0; filterX < MASK_X; filterX++){
          ...
          red += int(*(image_s + byte_per_pixel * (width * (imageY) + (imageX)) + 2) * mask[filterX][filterY]);
          green += int(*(image_s + byte_per_pixel * (width * (imageY) + (imageX)) + 1) * mask[filterX][filterY]);
          blue += int(*(image_s + byte_per_pixel * (width * (imageY) + (imageX)) + 0) * mask[filterX][filterY]);
          //std::cout << "what? 0.1" << std::endl;
        }
      }

      //std::cout << "what? 1" << std::endl;

      *(image_t + byte_per_pixel * (width * y + x) + 2) = std::min(std::max(int(factor * red + bias), 0), 255);
      *(image_t + byte_per_pixel * (width * y + x) + 1) = std::min(std::max(int(factor * green + bias), 0), 255);
      *(image_t + byte_per_pixel * (width * y + x) + 0) = std::min(std::max(int(factor * blue + bias), 0), 255);
    }
  }
